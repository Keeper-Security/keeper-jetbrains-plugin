...
/**
 * Simple unit tests for KeeperGetSecretAction
 */
class KeeperGetSecretActionTest {
    ...
    fun `test action creation`() {
        ...
        assertEquals("Action text should be correct", "Get Keeper Secret", action.templatePresentation.text)
    }
    ...
    fun `test action presentation properties`() {
        ...
        assertTrue("Text should contain Secret", presentation.text.contains("Secret"))
    ...
    fun `test multiple action instances are independent`() {
        val action1 = KeeperGetSecretAction()
        val action2 = KeeperGetSecretAction()
